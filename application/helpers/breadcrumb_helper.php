...
function create_breadcrumb()
{
	...
	elseif($ci->uri->segment(1) == 'items')
	{
        ...
        elseif($ci->uri->segment(1) == 'loans')
	{
		if ($ci->uri->segment(2) == false) //Main page
		{
			$items_home_link =create_current_page_url(lang('module_loans'));
		}
		else
		{
			$items_home_link = '<a href="'.site_url('loans').'">'.lang('module_loans').'</a>';
		}
		...
		if($ci->uri->segment(2) == 'view')
		{
			if ($ci->uri->segment(3) == -1)
			{
  				$return.=create_current_page_url(lang('loans_new'));
			}
			else
			{
  				$return.=create_current_page_url(lang('loans_update'));
			}
		}
		elseif($ci->uri->segment(2) == 'view_schedule')
		{
			$return.=create_current_page_url(lang('loans_view_schedule'));
		}
                elseif($ci->uri->segment(2) == 'add_attachment')
		{
			$return.=create_current_page_url(lang('loans_add_attachment'));
		}
                elseif($ci->uri->segment(2) == 'setting' || $ci->uri->segment(2) == 'update_rate')
		{
			$return.=create_current_page_url(lang('loans_setting'));
		}
	}elseif($ci->uri->segment(1) == 'pawns')
	{
		if ($ci->uri->segment(2) == false) //Main page
		{
			$items_home_link =create_current_page_url(lang('module_pawns'));
		}
		else
		{
			$items_home_link = '<a href="'.site_url('pawns').'">'.lang('module_pawns').'</a>';
		}
		...
		if($ci->uri->segment(2) == 'view')
		{
			if ($ci->uri->segment(3) == -1)
			{
  				$return.=create_current_page_url(lang('pawns_new'));
			}
			else
			{
  				$return.=create_current_page_url(lang('pawns_update'));
			}
		}
		elseif($ci->uri->segment(2) == 'view_schedule')
		{
			$return.=create_current_page_url(lang('pawns_view_schedule'));
		}
                elseif($ci->uri->segment(2) == 'add_attachment')
		{
			$return.=create_current_page_url(lang('pawns_add_attachment'));
		}
	...
	elseif($ci->uri->segment(1) == 'reports')
	{
		...
		elseif($ci->uri->segment(2) == 'summary_loans')
		{
			$return.=create_report_breadcrumb(lang('reports_loans_summary_report'));
		}
		elseif($ci->uri->segment(2) == 'detailed_loans')
		{
			$return.=create_report_breadcrumb(lang('reports_detailed_loans_report'));
		}elseif($ci->uri->segment(2) == 'graphical_summary_loans')
		{
			$return.=create_report_breadcrumb(lang('reports_graphical_reports'));
		}elseif($ci->uri->segment(2) == 'summary_pawns')
		{
			$return.=create_report_breadcrumb(lang('reports_pawns_summary_report'));
		}
		elseif($ci->uri->segment(2) == 'detailed_pawns')
		{
			$return.=create_report_breadcrumb(lang('reports_detailed_pawns_report'));
		}elseif($ci->uri->segment(2) == 'graphical_summary_pawns')
		{
			$return.=create_report_breadcrumb(lang('reports_graphical_reports'));
		}
	}
	...
	elseif($ci->uri->segment(1) == 'config')
	{
        ...
        elseif($ci->uri->segment(1) == 'home')
        {   
		if($ci->uri->segment(2) == 'loan')
		{
			$return.=create_current_page_url(lang('loans_view_all'));
		}else if($ci->uri->segment(2) == 'pawn')
		{
			$return.=create_current_page_url(lang('pawns_view_all'));
		}
	}

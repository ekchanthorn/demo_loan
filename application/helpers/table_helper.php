...
//Get all loans table to manage loans
function get_loans_manage_table($people,$controller)
{
	$CI =& get_instance();
	$table='<table class="tablesorter table table-bordered  table-hover" id="sortable_table">';	
	$controller_name=strtolower(get_class($CI));
	...
	if($controller_name=='loans')
	{
		$headers = array('<input type="checkbox" id="select_all" />', 
		lang('common_person_id'),
		lang('common_fullname'),
		lang('common_phone_number'),
		lang('loans_amount'),
		lang('loans_rate'),
		lang('loans_duration'),
		lang('loans_borrow_date'),
		lang('loans_option')
		);
	...
	$table.='</tr></thead><tbody>';
	$table.=get_loans_manage_table_data_rows($people,$controller);
	$table.='</tbody></table>';
	return $table;
}

/*
Gets the html data rows for the loans.
*/
function get_loans_manage_table_data_rows($people,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	...
	foreach($people->result() as $person)
	{
		$table_data_rows.= get_loans_data_row($person,$controller);
	}
...
function get_loans_data_row($person,$controller)
{
	$CI =& get_instance();
	$controller_name=strtolower(get_class($CI));
	$table_data_row='<tr>';	
	$amount = strtolower($person->currency) =='usd'?to_currency($person->amount):to_riel_currency($person->amount);
	$borrow_date = lang('common_'.date('D',strtotime($person->borrow_date))).', '.date('d',strtotime($person->borrow_date)).'-'.lang('common_'.date('M',strtotime($person->borrow_date))).'-'.date('Y',strtotime($person->borrow_date));
        $table_data_row.="<td><input type='checkbox'  id='loan_$person->loan_id' value='".$person->loan_id."'/></td>";
	$table_data_row.='<td>'.$person->loan_id.'</td>';
	$table_data_row.='<td>'.$person->first_name.' '.$person->last_name.'</td>';
	$table_data_row.='<td>'.$person->phone_number.'</td>';
	$table_data_row.='<td>'.$amount.'</td>';
	$table_data_row.='<td>'.$person->rate.'%</td>';
	$table_data_row.='<td>'.$person->duration.' '.  lang('loans_months').'</td>';
	$table_data_row.='<td>'.$borrow_date.'</td>';
	$table_data_row.='<td>
            '.anchor($controller_name."/view/$person->loan_id", lang('common_edit'),array('class'=>'update-loan','title'=>lang('common_edit'))).' | 
            '.anchor($controller_name."/view_schedule/$person->loan_id", lang('loans_view'),array('class'=>'view-loan','title'=>lang($controller_name.'_view'))).'
            </td>';
...
//Get all pawns table to manage pawns
...
/*
Gets the html data rows for the pawns.
*/
function get_pawns_manage_table_data_rows($people,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	...
	foreach($people->result() as $person)
	{
		$table_data_rows.= get_pawns_data_row($person,$controller);
	}
...
function get_pawns_data_row($person,$controller)
{
	$CI =& get_instance();
	$controller_name=strtolower(get_class($CI));
	$table_data_row='<tr>';	
	$amount = strtolower($person->currency) =='usd'?to_currency($person->amount):to_riel_currency($person->amount);
        ...
        if($person->pay_type==1){
            $duration_msg = $person->duration.' '. lang('common_day');
        }else if($person->pay_type==3){
             $duration_msg = lang('pawns_pay_type_half_month');
        }else{
            $durat = $person->duration.' '. lang('pawns_months');
            $duration_msg = $person->is_loan==1?$durat.' '.lang('pawns_the_same_loan'):$durat;
        }
        $borrow_date = lang('common_'.date('D',strtotime($person->start_date))).', '.date('d',strtotime($person->start_date)).'-'.lang('common_'.date('M',strtotime($person->start_date))).'-'.date('Y',strtotime($person->start_date));
        $table_data_row.="<td><input type='checkbox'  id='pawn_$person->pawn_id' value='".$person->pawn_id."'/></td>";
	$table_data_row.='<td>'.$person->pawn_id.'</td>';
	$table_data_row.='<td>'.$person->first_name.' '.$person->last_name.'</td>';
	$table_data_row.='<td>'.$person->phone_number.'</td>';
	$table_data_row.='<td>'.$amount.'</td>';
	$table_data_row.='<td>'.$person->rate.'%</td>';
	$table_data_row.='<td>'.$duration_msg.'</td>';
	$table_data_row.='<td>'.$borrow_date.'</td>';
	$table_data_row.='<td>
            '.anchor($controller_name."/view/$person->pawn_id", lang('common_edit'),array('class'=>'update-pawn','title'=>lang('common_edit'))).' | 
            '.anchor($controller_name."/view_schedule/$person->pawn_id", lang('pawns_view'),array('class'=>'view-pawn','title'=>lang($controller_name.'_view'))).'
            </td>';

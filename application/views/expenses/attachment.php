...
<div id="content-header" class="hidden-print">
    <h1 > <i class="fa fa-folder-open"></i>  <?php echo lang('loans_add_attachment');?>
     </h1>
</div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-bordered">
                            <tr>
                                ...
                                <th><?php echo lang('loans_option')?></th>
                            </tr>
                            <?php if($attachment->num_rows()>0){ 
                                ...
                                foreach ($attachment->result() as $att_file){ ?>
                                  <tr>
                                      <td><?php echo $i;?></td>
                                      <td> <a target="_blank" href="<?php echo base_url().'borrow_attachment/'.$this->uri->segment(3).'/'.$att_file->file_name;?>"><?php echo $att_file->file_name;?></a></td>
                                      <td>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                             <?php echo form_label(lang('loans_add_attachment') . ':', 'name', array('class' => 'col-sm-3 col-md-3 col-lg-2 control-label')); ?>
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                    ...
                    <div class="form-actions">
                        ...
                        <a href="<?php echo base_url().'loans/view_schedule/'.$this->uri->segment(3);?>" class="btn btn-danger"><?php echo lang('common_skip'); ?></a>
                    </div>
                    <?php echo form_close(); ?>
                </div>
            </div>
...
<script>
    ...
    $('.delete').click(function(){
        if(confirm('<?php echo lang("common_delete_file");?>')){
            $.ajax({
                type: "POST",
                url:"loans/delete_attachment",
                data:{
                    ids:$(this).attr('id')
                },
                success: function(msg) {
                   gritter(<?php echo json_encode(lang('common_success') . ' ' . lang('common_add')); ?>,<?php echo json_encode(lang('loans_attachment')); ?>, 'gritter-item-success', false, false); 
                   location.reload();
                },
                error: function (error) {
                   gritter(<?php echo json_encode(lang('common_error') . ' ' . lang('common_add')); ?>,<?php echo json_encode(lang('loans_attachment')); ?>, 'gritter-item-error', false, false); 
                }
            });
        }
    });
